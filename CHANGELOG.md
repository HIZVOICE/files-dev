...
## [5.9.2] — 2026-07-20 (versionCode 52)
### Changed
- **In-app folder icon now matches the launcher.** The generic folder icon (`ic_folder`, used throughout the file browser, tiles and archive list) is now the same **green glossy folder** as the launcher icon — layered lime→green→olive gradient with a glossy highlight sweep — instead of the old flat blue folder. Legible on both light and dark rows.
